...
private[codegen] final case class TypeConGen(
  ...
  private def renderProtoImpl(b: CodeBuilder): Unit = {
    val protoBound = if (paramNames.nonEmpty) {
      val bounds = paramNames.map(n => s"${n.capitalize}: daml_types::ToDamlProto")
      s"<${bounds.mkString(", ")}>"
    } else ""

    val typeNameWithGenerics = if (paramNames.nonEmpty) {
      s"$name<${paramNames.map(_.capitalize).mkString(", ")}>"
    } else name

    b.addEmptyLine()
    b.addLine(s"#[cfg(feature = \"proto\")]")
    b.addBlock(s"impl$protoBound daml_types::ToDamlProto for $typeNameWithGenerics {", "}") {
      b.addBlock(
        "fn to_proto(&self) -> daml_types::daml_proto_rs::com::daml::ledger::api::v2::Value {",
        "}",
      ) {

        cons match {
          case Ast.DataRecord(fields) =>
            renderRecordProto(b, fields)

          case Ast.DataVariant(variants) =>
            renderVariantProto(b, variants)

          case Ast.DataEnum(constructors) =>
            renderEnumProto(b, constructors)

          case Ast.DataInterface =>
            b.addLine("// Interface proto not supported yet")
        }
      }
    }
  }

  private def renderRecordProto(b: CodeBuilder, fields: ImmArray[(Name, Ast.Type)]): Unit = {
    b.addBlock("daml_types::daml_proto_rs::com::daml::ledger::api::v2::Value {", "}") {
      b.addBlock(
        "sum: Some(daml_types::daml_proto_rs::com::daml::ledger::api::v2::value::Sum::Record(",
        "))",
      ) {
        b.addBlock("daml_types::daml_proto_rs::com::daml::ledger::api::v2::Record {", "}") {
          b.addLine("record_id: None,")

          b.addBlock("fields: vec![", "],") {
            fields.foreach { case (fieldName, _) =>
              b.addBlock(
                "daml_types::daml_proto_rs::com::daml::ledger::api::v2::RecordField {",
                "},",
              ) {
                b.addLine(s"""label: "$fieldName".to_string(),""")
                // We delegate to the field's own to_proto implementation
                b.addLine(s"value: Some(self.$fieldName.to_proto()),")
              }
            }
          }
        }
  ...
  private def renderVariantProto(b: CodeBuilder, variants: ImmArray[(Name, Ast.Type)]): Unit = {
    b.addBlock("match self {", "}") {
      variants.foreach { case (variantName, _) =>
        // Match arm: Self::VariantName(v) => ...
        b.addBlock(
          s"Self::$variantName(v) => daml_types::daml_proto_rs::com::daml::ledger::api::v2::Value {",
          "},",
        ) {
          b.addBlock(
            "sum: Some(daml_types::daml_proto_rs::com::daml::ledger::api::v2::value::Sum::Variant(",
            "))",
          ) {
            b.addLine("Box::new(daml_types::daml_proto_rs::com::daml::ledger::api::v2::Variant {")
            b.addLine("  variant_id: None,")
            b.addLine(s"""  constructor: "$variantName".to_string(),""")
            b.addLine("  value: Some(Box::new(v.to_proto())),")
            b.addLine("})")
          }
        }
      }
  ...
  private def renderEnumProto(b: CodeBuilder, constructors: ImmArray[Name]): Unit = {
    b.addBlock("match self {", "}") {
      constructors.foreach { cName =>
        b.addBlock(
          s"Self::$cName => daml_types::daml_proto_rs::com::daml::ledger::api::v2::Value {",
          "},",
        ) {
          b.addBlock(
            "sum: Some(daml_types::daml_proto_rs::com::daml::ledger::api::v2::value::Sum::Enum(",
            "))",
          ) {
            b.addLine("daml_types::daml_proto_rs::com::daml::ledger::api::v2::Enum {")
            b.addLine("  enum_id: None,")
            b.addLine(s"""  constructor: "$cName".to_string(),""")
            b.addLine("}")
          }
        }
      }
